...
### Usage
```
python analyze_densenet.py --outprefix /path/to/output/folder/ --img_info /path/to/imageEmbeddingInfo.pkl --img_emd /path/to/imageEmbedding.pkl
```

### Required arguments for analyze_densenet.py:

`--outprefix` Full path to the folder where results will be saved in with unique file identifier. E.g. /path/to/output/folder/ 

`--img_info` Full path to pickled file containing info about each image. Example included in folder Examples
...
`--prefix` Prefix to use for generated similarity files files (ex: dim_1024_p_2_q_1)

`--gene_list` Full path to file containing genes to analyze. Example file img_x_ppi.661.genelist.npy is included in folder Examples
...
### Usage
```
python format_random_forest.py --outprefix /path/to/output/folder/ --go_input /path/to/go_input/genes.npy --gene_list /path/to/genelist.npy  --n2v_prefix dim1024_p_2_q_1 --densenet_prefix desenet_raw_1024D --n2v_emdfile /path/to/node2vec/emd.pkl --densenet_emdfile /path/to/imageEmbedding.pkl --rest_gp_input /path/to/restgenepairs.npy
```
...
`--go_input` Full path to file containing list of Gene Ontology terms size and genes (Example file `human_go_cc.no_hpa.symbol.termStats` in Example folder)

`--gene_list` Full path to file containing genes to analyze. Example file img_x_ppi.661.genelist.npy included in folder Examples

`--n2v_prefix` Prefix for node2vec generated similarity files (ex: dim_1024_p_2_q_1)

`--densenet_prefix` Prefix for densenet generated similarity files E.g. densenet_raw_1024D

`--n2v_emdfile` Full path to node2vec output emd file

`--densenet_emdfile` Full path to pickled file containing image embeddings from densenet

`--rest_gp_input` Full path to file with gene pairs not in Gene Ontology (Example file \ rest_genepair.npy in Example folder) 
...
### Usage 
```
python train_RF.py --outputdir /path/to/output/folder/ --sdir /path/to/train/test/data --n_estimators 1000 --max_depth 30 --max_features auto --n2v_prefix dim1024_p_2_q_1 --densenet_prefix desenet_raw_1024D
```
...
`--modelfname_prefix` Full path and prefix to model name prior to batch and fold # E.g. /path/to/folder/with/saved/models/densenet_raw_1024D.dim_1024_p_2_q_1.RF_maxDep_30_nEst_1000_maxFeat_auto
